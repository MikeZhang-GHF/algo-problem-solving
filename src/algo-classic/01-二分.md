...
> - 数学知识，GCD, LCM, 集合。
>  d1, d2, 满足要求的，所有的数字 - (d1的倍数 + d2的倍数 - LCM(d1, d2))
...
>  P1^a1 x P2^a2 x P3^a3... P1, P2, P3都是质因子,阶乘后有K个0的数字，取决于这个数分解质因子后，min(cnt(2), cnt(5))，分解质因子算法， x/P + x/P^2 + x/P^3，这里可以看出质因子5的个数必然比2个数少。答案就取决于质因子5的个数。
